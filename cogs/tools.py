...
async def safe_screenshot(url: str, timeout_ms: int = 15000) -> io.BytesIO | None:
    try:
...
class RunNodeJS(discord.ui.Modal, title='NodoJSを実行'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        headers = {
            ...
            'priority': 'u=1, i',
            'referer': 'https://www.mycompiler.io/ja/new/nodejs',
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post("https://www.mycompiler.io/exec/run/nodejs", headers=headers, json=json_data) as response:
                text = await response.text()
                js = text.replace(":", "", 1)
                await interaction.followup.send(embed=discord.Embed(title="NodoJSの実行結果", description=f"```{json.loads(js)["payload"]}```", color=discord.Color.blue()))
...
class RunCPlapla(discord.ui.Modal, title='C++を実行'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        headers = {
            ...
            'origin': 'https://www.mycompiler.io',
            'priority': 'u=1, i',
            'referer': 'https://www.mycompiler.io/ja/new/cpp',
            'sec-ch-ua': '"Google Chrome";v="129", "Not=A?Brand";v="8", "Chromium";v="129"',
            'sec-ch-ua-mobile': '?0',
            'sec-ch-ua-platform': '"Windows"',
            'sec-fetch-dest': 'empty',
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post("https://www.mycompiler.io/exec/run/cpp", headers=headers, json=json_data) as response:
                text = await response.text()
                js = text.replace(":", "", 1).replace(".", "", 1)
                await interaction.followup.send(embed=discord.Embed(title="C++の実行結果", description=f"```{json.loads(js)["payload"]}```", color=discord.Color.blue()))
...
class RunCSharp(discord.ui.Modal, title='C#を実行'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        headers = {
            ...
            'origin': 'https://www.mycompiler.io',
            'priority': 'u=1, i',
            'referer': 'https://www.mycompiler.io/ja/new/csharp',
            'sec-ch-ua': '"Google Chrome";v="129", "Not=A?Brand";v="8", "Chromium";v="129"',
            'sec-ch-ua-mobile': '?0',
            'sec-ch-ua-platform': '"Windows"',
            'sec-fetch-dest': 'empty',
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post("https://www.mycompiler.io/exec/run/csharp", headers=headers, json=json_data) as response:
                text = await response.text()
                js = text.replace(":", "", 1)
                await interaction.followup.send(embed=discord.Embed(title="C#の実行結果", description=f"```{json.loads(js)["payload"]}```", color=discord.Color.blue()))
...
class ToolCog(commands.Cog):
    ...
    async def on_message_afk(self, message: discord.Message):
        ...
        if dbfind is None:
            return
        await message.reply(embed=discord.Embed(title="AFKを解除しました。", description=f"{dbfind["Reason"]}", color=discord.Color.green()).set_footer(text="このメッセージを5秒後に削除されます。"), delete_after=5)
        await db.delete_one({
    ...
    @afk_set.command(name="nmap", description="開いているポートを取得します。")
    @commands.cooldown(2, 10, commands.BucketType.guild)
    async def nmap(self, ctx: commands.Context, ip: str):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://api.voids.top/nmap/{ip}") as response:
                ...
                if ps == []:
                    return await ctx.reply(embed=discord.Embed(title="<:Error:1362271424227709028> 開いているポートはありません。", color=discord.Color.red()))
                return await ctx.reply(embed=discord.Embed(title="<:Success:1362271281302601749> 開いているポート一覧です。", color=discord.Color.green(), description="\n".join(ps)))
